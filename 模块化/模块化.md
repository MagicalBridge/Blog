...
require('./foo.js')
```
在这里，index.js 是执行的入口，它加载了 foo.js foo.js 和bar.js 之间存在循环依赖。让我们观察foo.js 和 bar.js 中的代码，理想状态下我们希望二者能够导入正确的值，并在控制台上输出：
...
## AMD 和 CMD 的区别
从sea.js 和 require.js 例子中可以看出：

1、CMD 推崇的标准是 依赖就近，AMD 推崇的是依赖前置，看两个项目中的main.js
...
2、对于依赖的模块，AMD 是提前执行 ，CMD是延迟执行，看看这两个项目的打印顺序
...
跟sea.js 一样 也就是在require的时候才去 加载该模块 加载完之后接着执行

## CommonJS 与 AMD
CommonJS 规范加载模块是同步的，也就是说，只有加载完成，才能执行后面的操作
AMD 规范是是非同步加载模块，允许指定回调函数

由于node.js主要用于服务器编程，模块文件一般都已经存在于本地硬盘，所以加载起来比较快，不用考虑非同步加载的方式，所以CommonJS规范比较适用

但是 如果是浏览器环境，要从服务器端加载模块，这时就必须采用非同步方式，因此浏览器端一般采用AMD规范。
